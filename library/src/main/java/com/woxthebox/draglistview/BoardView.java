...
public class BoardView extends HorizontalScrollView implements AutoScroller.AutoScrollListener {
    ...
    private ArrayList<ClickedItem> clickedList = new ArrayList();
    private ArrayList<Integer> clickedPosSelectedList = new ArrayList();
    ...
    protected void onFinishInflate() {
        ...
        if (isPortrait) {
            ...
            float screen = getContext().getResources().getDisplayMetrics().widthPixels;
            mColumnWidth = (int) ((screen/3) - 10);
        }
        ...
        //Set background to all columns together
        //mColumnLayout.setBackground(ContextCompat.getDrawable(getContext(),R.drawable.gray_rounded));
    ...
    public void deleteColumn(int pos) {
        mColumnLayout.removeViewAt(pos);
        mHeaders.remove(pos);
        mFooters.remove(pos);
        mLists.remove(pos);
    }
    ...
    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
    public void addToClickedList(Boolean payed) {
        clickedList.add(new ClickedItem(false, payed));
    }

    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
    public void deleteClickedList() {
        clickedList.clear();
        clickedPosSelectedList.clear();
    }

    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
    public ArrayList<Integer> getClickedList() {
        return clickedPosSelectedList;
    }
    ...
    private DragItemRecyclerView addColumnTo(int index, @NonNull ColumnProperties columnProperties) {
        ...
        layout.setBackgroundColor(columnProperties.getColumnBackgroundColor()); // Coloca el fondo de todas las columnas
        ...
        setUpColumnClickListener(columnProperties.getColumnDragView(), recyclerView);
        ...
        if (columnDragView != null) {
            columnDragView.setOnLongClickListener(new OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    //Activate dragColumn, columnDrag
    ...
    private void setUpColumnClickListener(View columnDragView, final DragItemRecyclerView recyclerView ) {
        if (columnDragView != null) {
            columnDragView.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                   int column = getColumnOfList(recyclerView);
                    //Toast.makeText(getContext(), "Column: " + column, Toast.LENGTH_SHORT).show();
                    View child = mColumnLayout.getChildAt(column);
                    if (!clickedList.get(column).isPayed) {
                        if (!clickedList.get(column).isClicked) {
                            child.setBackground(ContextCompat.getDrawable(getContext(), R.drawable.blue_rounded));
                            clickedPosSelectedList.add(column);
                        } else {
                            child.setBackground(ContextCompat.getDrawable(getContext(), R.drawable.gray_rounded));
                            int pos = -1;
                            for (int i = 0; i < clickedPosSelectedList.size(); i++) {
                                if (clickedPosSelectedList.get(i) == column) {
                                    pos = i;
                                    break;
                                }
                            }
                            if (pos != -1) {
                                clickedPosSelectedList.remove(pos);
                            }
                        }
                        clickedList.get(column).setClicked(!clickedList.get(column).getClicked());
                    }
                }
            });
    ...
    private void updateBoardSpaces() {
        ...
        for (int childPosition = 0; childPosition < columnCount; childPosition++) {
            ...
            if (!clickedList.get(childPosition).isPayed) {
                if (clickedList.get(childPosition).isClicked) {
                    child.setBackground(ContextCompat.getDrawable(getContext(), R.drawable.blue_rounded));
                } else {
                    child.setBackground(ContextCompat.getDrawable(getContext(), R.drawable.gray_rounded));
                }
            } else {
                child.setBackground(ContextCompat.getDrawable(getContext(), R.drawable.orange_rounded));
            }
